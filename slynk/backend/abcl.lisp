...
;; TODO: test! GD!
(defun listen (stream)
  ;; Check if the stream has some data ready to read from
  (and (typep stream 'SYSTEM::SOCKET-STREAM)
       (let* ((socket (java:jcall
                       (java:jmethod "org.armedbear.lisp.SocketStream" "getJavaSocket") stream))
              (get-input-stream-method (java:jmethod "java.net.Socket" "getInputStream"))
              (input-stream (java:jcall get-input-stream-method socket))
              (available-method (java:jmethod "java.io.InputStream" "available")))
         (not (zerop (java:jcall available-method input-stream))))))
